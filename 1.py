...
def compute_fitnesses_for_selection(population, func):
    """Converts minimization objective to maximization fitness"""
...
def run_experiments(func, bounds, func_name, filename):
    schemes = [
        ('FPS', 'Truncation'), ('RBS', 'Truncation'), ('Tournament', 'Truncation'),
        ('FPS', 'Tournament'), ('RBS', 'Tournament'), ('Tournament', 'Tournament')
    ]
    ...
    plt.suptitle(f"Performance for {func_name}")
    ...
    for p_scheme, s_scheme in schemes:
        ...
        all_results[label] = (mean_bsf, mean_acp)
        ...
        plt.subplot(1, 2, 1)
        plt.plot(range(1, generations+1), mean_bsf, label=label)
        plt.subplot(1, 2, 2)
        plt.plot(range(1, generations+1), mean_acp, label=label)
    ...
    plt.subplot(1, 2, 2)
    plt.title("Average of Current Population (ACP)")
    plt.xlabel("Generation")
    plt.ylabel("Objective Value (Lower is Better)")
    plt.legend()
    plt.grid(True)
...
bounds_f1 = ((-5, 5), (-5, 5))
run_experiments(function_1, bounds_f1, "Function 1 (Sphere)", "results_func1.csv")


bounds_f2 = ((-2, 2), (-1, 3))
run_experiments(function_2, bounds_f2, "Function 2 (Rosenbrock)", "results_func2.csv")