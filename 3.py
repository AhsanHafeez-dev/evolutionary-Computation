...
def run_experiments(func, bounds, func_name, safe_name):
    ...
    csv_header = ["Generation", "Parent_Scheme", "Survival_Scheme", "Average_BSF", "Average_ACP"]
    csv_header.extend([f"Run{i+1}_ACP" for i in range(runs)])
    csv_data = [csv_header]
    ...
    for p_scheme, s_scheme in schemes:
        ...
        for gen in range(generations):
            row = [gen+1, p_scheme, s_scheme, mean_bsf[gen], mean_acp[gen]]
            
            row.extend(acp_matrix[:, gen].tolist())
            csv_data.append(row)
